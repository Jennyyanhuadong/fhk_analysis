...
- Picking one of these tickers, how can you match that ticker to the GVKEY value I have provided? Do you agree with my approach?
  ...
  |(a wholly subsidiary of JEF, |         |	        |           |	         |          |	         |    |
  |spun off ITG in 1999)        |         |	        |           |	         |          |	         |    |
...
get_ff_ind <- function(num = 48) {
  ...
  # fileext gives the tempfile extension"
    ...
    # The number of industries 1-48, has 3 spaces. 
    # The name of the industries has 7 spaces. 
    # The characters after the name are defined as "sic_range"
...
reg_year_fe <- function(y, firm_fe = FALSE, cl_2 = TRUE) {
    model <- paste0(y, " ~ pilot_during + pilot_post + pilot + ", 
                    # I find that if we use " ~ pilot_during + pilot_post + pilot + during + post + " by strictly following FHK's model, the code also work. 
...
fm_year_fe_table <-
  stargazer(fm_year_fe, 
          ...
          # type = 'text',
          title = "Change filtering code in producing 'controls(a)' with cl2")   # I add the table title.
...
fm_year_fe_al1 <- lapply(lhs, reg_year_fe_al1, firm_fe = FALSE, cl_2 = TRUE)
stargazer(fm_year_fe_al1, 
          omit=paste0("^(", paste(controls, collapse="|"), ")$"),
          column.labels = gsub("_", "\\\\_", lhs),
         header=FALSE,
         keep.stat = c("n", "rsq", "chi2"),
         # type = 'text',
         title = "Three multiply with cl2'")

# Two multiply
fm_year_fe_al2 <- lapply(lhs, reg_year_fe_al2, firm_fe = FALSE, cl_2 = TRUE)
stargazer(fm_year_fe_al2, 
          omit=paste0("^(", paste(controls, collapse="|"), ")$"),
          column.labels = gsub("_", "\\\\_", lhs),
         header=FALSE,
         keep.stat = c("n", "rsq", "chi2"),
         # type = 'text',
         title = "Two multiply with cl2'")
```
